...
'var SPREADSHEET_ID = "1uBan7gznXd54U_PEK_J4SSHYhThLlAQb9ZAoLz-o4xc";\n' +
...
'    var ss = SpreadsheetApp.openById(SPREADSHEET_ID);\n' +
'    return ContentService\n' +
'      .createTextOutput(JSON.stringify({ok:true,grupos:_readSheet(ss,"Grupos"),clases:_readSheet(ss,"Clases")}))\n' +
'      .setMimeType(ContentService.MimeType.JSON);\n' +
'  } catch(err) {\n' +
'    return ContentService\n' +
'      .createTextOutput(JSON.stringify({ok:false,error:err.message}))\n' +
...
'}' +
      ...
      document.getElementById('btnSync').onclick = async function () {
        ...
        if (!grupos.length) { Toast.warning('No hay grupos para sincronizar.'); return; }
        var gMap = {};
        ...
        .then(function (json) {
          if (json && json.ok) {
            Toast.success('✓ Enviado: ' + grupos.length + ' grupos, ' + clases.length + ' clases. Verifica tu hoja (pestañas "Grupos" y "Clases").');
          } else {
      ...
      document.getElementById('btnPull').onclick = async function () {
        ...
        try {
          ...
          if (!gRows.length && !cRows.length) { Toast.warning('La hoja está vacía.'); return; }
          ...
          // Reconstruir grupos directamente desde la pestaña "Grupos" (nombre, asignatura, grado, color)
          var gruposNuevos = [];
          ...
          // Reconstruir clases y enlazarlas al grupo correspondiente (o crearlo si no estaba en la pestaña Grupos)
          var clasesNuevas = [];
