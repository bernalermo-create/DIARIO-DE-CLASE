...
var ClassesService = (function () {
  ...
  return {
    ...
    /**
     * Fallas totales por grupo dentro de un rango de fechas (para un periodo académico).
     * Recorre TODOS los grupos, no solo uno.
     */
    async absencesByGroupInRange(startDate, endDate) {
      var [clases, grupos] = await Promise.all([this.getAll(), GroupsService.getAll()]);
      var gMap = {};
      grupos.forEach(function (g) { gMap[g.id] = g; });

      var tally = {}; // groupId -> { faltas, presentes, sesiones }
      clases.forEach(function (c) {
        if (c.fecha < startDate || c.fecha > endDate) return;
        if (!Array.isArray(c.asistenciaLista) || !c.asistenciaLista.length) return;
        if (!tally[c.groupId]) tally[c.groupId] = { faltas: 0, presentes: 0, sesiones: 0 };
        tally[c.groupId].sesiones++;
        c.asistenciaLista.forEach(function (a) {
          if (a.presente) tally[c.groupId].presentes++;
          else tally[c.groupId].faltas++;
        });
      });

      var rows = Object.keys(tally).map(function (gid) {
        var g = gMap[gid] || {};
        var t = tally[gid];
        return { groupId: gid, nombre: g.nombre || '(grupo eliminado)', asignatura: g.asignatura || '', faltas: t.faltas, presentes: t.presentes, sesiones: t.sesiones };
      });
      rows.sort(function (a, b) { return b.faltas - a.faltas; });
      return rows;
    },
