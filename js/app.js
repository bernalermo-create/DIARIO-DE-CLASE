...
var Router = (function () {
  ...
  return {
    ...
    buildBottomNav() {
      ...
      if (!el) return;
      el.innerHTML = NAV_ITEMS.map(function (n) {
        return '<a href="#" class="bottom-nav-item" data-view="' + n.id + '">' +
          '<span class="bottom-nav-emoji">' + n.emoji + '</span>' +
          '<span class="bottom-nav-label">' + n.label.replace('Configuración', 'Config') + '</span>' +
        '</a>';
      }).join('');
      el.querySelectorAll('.bottom-nav-item').forEach(function (link) {
        link.addEventListener('click', function (e) {
          e.preventDefault();
          Router.go(link.dataset.view);
        });
...
async function _autoSyncOnOpen() {
  try {
    ...
    console.warn('[App] No se pudo sincronizar automáticamente (se continúa con los datos locales):', err);
    return null;
  }
}
...
async function initApp() {
  try {
    ...
    var accent    = (await DB.getCfg('colorAccent'))     || 'indigo';
    ...
    // 12. Sincronización silenciosa cada 5 minutos mientras la app queda abierta
    ...
    // 13. Red de seguridad: si hay cambios sin subir y la pestaña se va a
    // segundo plano o se cierra, intentar subirlos ya mismo (sin bloquear).
    document.addEventListener('visibilitychange', function () {
      if (document.hidden) {
        DB.isDirty().then(function (dirty) {
          if (dirty) SheetsSyncService.pushInBackground();
        });
      }
    });
