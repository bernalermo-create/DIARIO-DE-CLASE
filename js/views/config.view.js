...
'}' +
      ...
      document.getElementById('btnSync').onclick = async function () {
        ...
        var header = ['Grupo','Asignatura','Grado','Fecha','Periodo','Tema','Desarrollo','Tarea','Fecha entrega','Revisada','Observaciones'];
      ...
      document.getElementById('btnPull').onclick = async function () {
        ...
        try {
          ...
          // Mapear columnas por nombre de encabezado (compatible con hojas viejas sin "Grado")
          var hdr = (json.header || []).map(function (h) { return (h || '').toString().trim().toLowerCase(); });
          function col(name, fallbackIdx) {
            var i = hdr.indexOf(name);
            return i > -1 ? i : fallbackIdx;
          }
          var iGrupo  = col('grupo', 0),      iAsig    = col('asignatura', 1),
              iGrado  = col('grado', -1),      iFecha   = col('fecha', iGrado > -1 ? 3 : 2),
              iPeriodo= col('periodo', iGrado > -1 ? 4 : 3), iTema = col('tema', iGrado > -1 ? 5 : 4),
              iDesarr = col('desarrollo', iGrado > -1 ? 6 : 5), iTarea = col('tarea', iGrado > -1 ? 7 : 6),
              iFechaT = col('fecha entrega', iGrado > -1 ? 8 : 7), iRev = col('revisada', iGrado > -1 ? 9 : 8),
              iObs    = col('observaciones', iGrado > -1 ? 10 : 9);

          // Reconstruir grupos únicos (Grupo + Asignatura + Grado) y sus clases
          var gruposNuevos = [];
          var gMap = {};
          var clasesNuevas = [];

          rows.forEach(function (r) {
            var nombre     = (r[iGrupo] || '').toString().trim();
            var asignatura = (r[iAsig] || '').toString().trim();
            var grado      = iGrado > -1 ? (r[iGrado] || '').toString().trim() : '';
            if (!nombre) return;
            ...
            clasesNuevas.push({
              id:            Utils.id(),
              groupId:       gMap[key].id,
              fecha:         toISODate(r[iFecha]),
              periodo:       (r[iPeriodo] || '').toString(),
              tema:          (r[iTema] || '').toString(),
              desarrollo:    (r[iDesarr] || '').toString(),
              tarea:         (r[iTarea] || '').toString(),
              fechaTarea:    toISODate(r[iFechaT]),
              tareaRevisada: (r[iRev] || '').toString().trim().toLowerCase() === 'sí' || (r[iRev] || '').toString().trim().toLowerCase() === 'si' ? 1 : 0,
              observaciones: (r[iObs] || '').toString(),
              asistencia:    '',
              destacado:     0,
