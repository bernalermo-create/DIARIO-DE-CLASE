...
/* ── Carga automática desde Sheets (primera vez, sin datos locales) ── */
async function _autoLoadFromSheetsIfEmpty() {
  try {
    var already = await DB.getCfg('autoSyncAttempted');
    if (already) return null; // ya se intentó antes en este navegador, no repetir solo

    await DB.setCfg('autoSyncAttempted', true);

    var grupos = await DB.getAll('grupos');
    if (grupos && grupos.length) return null; // ya hay datos locales, no tocar nada
    ...
    if (r.ok && !r.empty) {
      console.log('[App] Datos cargados automáticamente desde Google Sheets: ' + r.gruposCount + ' grupos, ' + r.clasesCount + ' clases.');
      return r;
    }
    return null;
  } catch (err) {
    console.warn('[App] No se pudo autocargar desde Sheets (se continúa sin datos):', err);
    return null;
  }
}
...
async function initApp() {
  try {
    ...
    // 8.5. Cargar automáticamente desde Sheets si es la primera vez en este navegador y no hay datos locales
    var autoLoaded = await _autoLoadFromSheetsIfEmpty();
    ...
    if (autoLoaded) {
      setTimeout(function () {
        if (typeof Toast !== 'undefined') {
          Toast.success('✓ Se cargaron tus datos guardados en Google Sheets (' + autoLoaded.gruposCount + ' grupos, ' + autoLoaded.clasesCount + ' clases, ' + autoLoaded.estudiantesCount + ' estudiantes).');
        }
      }, 400);
    }
