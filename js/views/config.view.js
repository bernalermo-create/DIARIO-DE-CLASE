...
'    if (d.horario) _writeSheet(ss,"Horario",d.horario.header,d.horario.rows,[5,6]);\n' +
...
'}' +
        ...
        /* ── Fallas por curso ── */
        '<div class="card mt-3">' +
          '<h3 class="section-title">📊 Fallas totales por curso</h3>' +
          '<div class="field">' +
            '<label class="field-label">Periodo</label>' +
            '<select class="select" id="absPeriodSelect"></select>' +
          '</div>' +
          '<div id="absTable" style="margin-top:10px"></div>' +
          '<button class="btn btn-secondary btn-sm mt-2" id="btnExportAbs" style="display:none">⬇ Exportar CSV</button>' +
        '</div>' +
      ...
      document.getElementById('btnSavePeriods').onclick = async function () {
        ...
        await refreshAbsencesSection();
      };

      /* ── Fallas totales por curso ── */
      var absSelectEl = document.getElementById('absPeriodSelect');
      var absTableEl  = document.getElementById('absTable');
      var absExportBtn = document.getElementById('btnExportAbs');

      async function populateAbsPeriodSelect() {
        var periods = await PeriodsService.getAll();
        var current = await PeriodsService.getCurrentPeriod();
        absSelectEl.innerHTML = periods.map(function (p, i) {
          var isCurrent = current && current.nombre === p.nombre && current.inicio === p.inicio;
          return '<option value="' + i + '"' + (isCurrent ? ' selected' : '') + '>' + Utils.esc(p.nombre) + ' (' + Utils.dateShort(p.inicio) + ' – ' + Utils.dateShort(p.fin) + ')</option>';
        }).join('');
      }

      async function refreshAbsencesSection() {
        await populateAbsPeriodSelect();
        var periods = await PeriodsService.getAll();
        var idx = +absSelectEl.value || 0;
        var period = periods[idx];
        if (!period) { absTableEl.innerHTML = ''; absExportBtn.style.display = 'none'; return; }

        var rows = await ClassesService.absencesByGroupInRange(period.inicio, period.fin);
        if (!rows.length) {
          absTableEl.innerHTML = '<p class="text-sm text-muted">Sin asistencia registrada en este periodo todavía.</p>';
          absExportBtn.style.display = 'none';
          return;
        }
        absTableEl.innerHTML = rows.map(function (r) {
          return '<div class="flex justify-between items-center" style="padding:8px 4px;border-bottom:1px solid var(--border)">' +
            '<div><p style="font-size:14px;font-weight:600;margin:0">' + Utils.esc(r.nombre) + (r.asignatura ? ' — ' + Utils.esc(r.asignatura) : '') + '</p>' +
            '<p class="text-xs text-muted" style="margin:2px 0 0">' + r.sesiones + ' ' + Utils.plural(r.sesiones, 'clase') + ' con asistencia</p></div>' +
            '<span style="font-size:16px;font-weight:800;color:' + (r.faltas > 0 ? 'var(--danger)' : 'var(--primary)') + '">' + r.faltas + '</span>' +
          '</div>';
        }).join('');
        absExportBtn.style.display = '';
      }
      await refreshAbsencesSection();
      absSelectEl.addEventListener('change', refreshAbsencesSection);
      absExportBtn.onclick = async function () {
        var periods = await PeriodsService.getAll();
        var period = periods[+absSelectEl.value || 0];
        var rows = await ClassesService.absencesByGroupInRange(period.inicio, period.fin);
        var header = ['Curso', 'Asignatura', 'Faltas totales', 'Sesiones con asistencia'];
        var csvRows = rows.map(function (r) { return [r.nombre, r.asignatura, r.faltas, r.sesiones]; });
        var csv = '\uFEFF' + [header, ...csvRows].map(function (r) {
          return r.map(function (v) { var s = String(v == null ? '' : v).replace(/"/g, '""'); return /[,"\n]/.test(s) ? '"' + s + '"' : s; }).join(',');
        }).join('\r\n');
        Utils.downloadBrowser('fallas-' + period.nombre.replace(/[^a-z0-9]/gi, '_') + '.csv', csv, 'text/csv;charset=utf-8');
      };
