  ...
  return {
    ...
    async exportCSV(groupId) {
      ...
      var header = ['Grupo','Asignatura','Fecha','Periodo','Tema','Desarrollo',
                    'Tarea','Fecha entrega','Revisada','Observaciones','Asistencia','Presentes','Total','Destacada'];
      var rows = clases.map(function (c) {
        var g = gMap[c.groupId] || {};
        var lista = Array.isArray(c.asistenciaLista) ? c.asistenciaLista : [];
        ...
        return rowToCSV([
          ...
          lista.length ? presentes : '', lista.length ? lista.length : '',
          +c.destacado ? 'Sí' : 'No'
        ]);
      });
    ...
    /** Exporta el informe acumulado de asistencia de un grupo a CSV */
    async exportAttendanceReport(groupId) {
      var g = await GroupsService.getById(groupId);
      var report = await ClassesService.attendanceReport(groupId);

      var header = ['Estudiante', 'Presentes', 'Ausentes', 'Total sesiones registradas', '% Asistencia'];
      var rows = report.rows.map(function (r) {
        return rowToCSV([r.nombre, r.presentes, r.ausentes, r.total, r.pct + '%']);
      });

      var csv = '\uFEFF' + [rowToCSV(header), ...rows].join('\r\n');
      var filename = 'asistencia-' + (g ? g.nombre.replace(/[^a-z0-9]/gi, '_') : 'grupo') + '-' + Utils.today() + '.csv';
      await shareOrDownload(filename, csv, 'text/csv;charset=utf-8');
